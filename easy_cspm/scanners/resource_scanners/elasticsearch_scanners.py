...
class ElasticsearchDomainScanner(BaseScanner):
    """Scanner for Elasticsearch/OpenSearch domains"""
    ...
    def scan(self):
        """Scan Elasticsearch/OpenSearch Domains in the current region"""
        try:
            client = self.aws_client.get_client('opensearch')
            
            domain_count = 0
            resource_ids = []
            
            # List domains
            list_response = client.list_domain_names()
            ...
            for domain_info in list_response.get('DomainNames', []):
                ...
                try:
                    ...
                    domain_response = client.describe_elasticsearch_domain(DomainName=domain_name)
                    domain = domain_response.get('DomainStatus', {})
                    
                    # Get domain config
                    config_response = client.describe_elasticsearch_domain_config(DomainName=domain_name)
                    ...
                    try:
                        arn = domain.get('ARN')
                        if arn:
                            tags_response = client.list_tags(ARN=arn)
            ...
            logger.info(f"Discovered {domain_count} OpenSearch domains in account {self.account_id} region {self.region}")
            return resource_ids
            
        except botocore.exceptions.ClientError as e:
            error_msg = str(e)
            logger.error(f"Error scanning OpenSearch domains: {error_msg}")
